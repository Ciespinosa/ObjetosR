...
<button name="button" onclick="https://ciespinosa.github.io/IntroduccionR">Regresar
...
Como ven R es muy potente y podemos hacer muchas cosas con él, cada uno de los objetos pueden ser operados, en este caso a los objetos los hemos multiplicado, dividido o sumado. Verán más adelante que las operaciones pueden ser mucho más complejas.
...
```
Como vemos este objeto es un vector numérico, doble. La función `mode` nos da el tipo más general del vector mientras que la función `typeof` nos da el tipo más específico. En este caso la función `class` nos da también el tipo más general.
...
La función `names` me devuelve los nombres de los elementos, cuando imprimimos nuestro vector se muestran los datos y los nombres.

```{r}
names(DaCN)
```
Si utilizo la función `names` con un data.frame me devuelve los nombres de las columnas. Al igual que en el caso de los vectores puedo usar esta misma función para cambiar los nombres.
